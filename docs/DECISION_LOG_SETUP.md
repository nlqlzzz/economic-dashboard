...
Migrationは判断区分、20/60/120営業日、理由タグ1〜3個、判断モード、Human source、正の参考価格、JSON objectにDB制約を設けます。過去判断は更新せず、新しい行を追加します。将来の訂正は`supersedes_decision_id`で関連付けられる構造です。誤操作を実判断として残しにくくするため、画面の判断モード初期値は「仮想判断」です。
...
1306.Tの判断基準日後20・60・120番目の取引観測日を共通終点とし、個別株にも同じ日付の価格がある場合だけ両リターンとTOPIX超過を計算します。未到来は「評価待ち」、品質停止・長期欠損・共通終点の価格欠損は「評価不能」です。後日の別価格へ飛ばして代用しません。

保存済み参考価格は「判断時に画面へ表示された値」の監査用Snapshotとして変更しません。Yahoo Financeの調整後系列は取得時点によって過去値が変わり得るため、前向きリターンの分母と分子には、評価時に同時取得した同一系列の判断基準日価格と共通終点価格を使います。保存値と現在系列の基準日価格に差がある場合は診断として表示しますが、差率だけで評価不能にはしません。補正係数は推測しません。
