...
## PR-A: 比較可能性と判定の修正（2026-09-12）

- `CurPerSt`、`CurPerEn`、`CurFYSt`、`CurFYEn`を正規化し、最新決算は一つの対象期・会計基準・連結区分・通貨・単位のcohortとして選ぶ。最新期に欠けるmetricを過去期から補完しない。
- 売上・利益は累計定義と期間が一致するときだけ単独四半期を差分導出する。EPSは加重平均株式数の違いを考慮し、差分導出せず、取得した累計または通期開示値として扱う。
- 前年が負・ゼロの場合は通常の前年比率を表示しない。黒字転換、赤字転落、赤字縮小、赤字拡大、横ばい、比較不能を共通比較区分として分析・UI・解釈に使う。
- Financial Summaryの売上・利益・会社予想の金額は円建てとして正規化し、カード・軸・ツールチップで共通の兆円／億円formatterを使う。EPSは円/株の値として別表示する。
- 会社予想は直近実績と同年度以降の対象年度を選び、開示日の同じforecast cohortだけを表示する。欠損を取り下げと推測しない。
