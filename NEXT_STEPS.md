...
1. Price vs Fundamentals
   - SOXなどの市場価格と、台湾受注・韓国輸出・日本出荷の方向が一致しているかを説明可能なルールで表示する。
   - 「実需と株価が同方向」「期待先行の可能性」「実需に対して株価が弱い」「シグナル混在」までにとどめ、売買判断として表示しない。
2. Global Semiconductor Demand Pulse
   - Taiwan Orders、Korea Exports、Japan Production / Shipments / Inventory / Capexを統合する。
   - Strong / Improving / Mixed / Weakeningの根拠となった系列を開示し、ブラックボックスの総合点は作らない。
3. Historical Validation
   - 台湾・韓国の履歴公表日を確保してから、公表日基準のリード・ラグと条件付き将来リターンを追加する。
   - 対象月を利用可能日として扱わず、実公表日が不明な観測は厳密な検証へ使用しない。
