...
1. **A. 決算データと業績判定の正確性修正（完了・PR69/PR70）**
...
2. **B. 価格品質・時点整合・過去検証の統一（PR71マージ済み、期間整合補修は追加PR）**
   - 個別銘柄、Core20マーケットマップ、マクロ感応度で同じ価格品質判定を利用する。
   - 相対リターンは銘柄とベンチマークの共通起点・終点だけで比較し、欠損の前方補完をしない。
   - 公表日が異なる系列は各系列の直前の有効な公表値と比較し、初回観測を判定不能として保持する。
   - 公表後評価は日付のみの開示では翌営業日以降の終値を起点にし、暦月の満了前・終点欠損を除外理由として表示する。
   - 公表時刻と改定履歴が未確認の分析を「厳密」「未来情報なし」とは表示しない。
   - 追加補修では、日次変化量とResidualの起点日を比較先と一致させ、ペア価格品質処理後に共通評価日を再決定する。
   - 暦月満了後の起点・終点探索は7暦日の許容幅を設け、長期欠損を短い評価期間へ混ぜない。
3. **C. 日本株を主入口にした銘柄分析の画面統合（次PR）**
